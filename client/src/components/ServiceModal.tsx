...
export default function ServiceModal({ id, service, onClose }: ServiceModalProps) {
  ...
  // Perfectly synchronized animations for smooth transitions
  const animationDuration = prefersReducedMotion ? 0 : 0.35;
  
  // Modal spring settings - optimized for smoothness
  const modalSpring = { 
    type: 'spring', 
    stiffness: 300, 
    damping: 25, 
  ...
  // Backdrop animation - perfectly timed with modal
  const backdropVariants = {
    ...
    exit: { 
      ...
      transition: { 
        duration: animationDuration,
        ease: 'easeIn',
        delay: 0 // No delay to keep in sync
      }
    }
  };
  ...
  return createPortal(
    <LazyMotion features={domAnimation}>
      <AnimatePresence mode="wait" onExitComplete={restoreFocus}>
        {/* Backdrop and Modal in separate layers but synchronized timing */}
        <>
          {/* Backdrop Layer */}
          <motion.div
            key="backdrop"
          ...
          >
            <motion.div
              ref={modalRef}
              layoutId={`service-${id}`}
              className="relative w-full max-w-4xl bg-[var(--brand-bg)] shadow-2xl pointer-events-auto"
              style={{ 
